...
class Seance extends Model {
    ...
    protected $fillable = ['matiere_id','salle_id','professeur_id','annee_scolaire_id','emploi_du_temps_id','debut','fin','type','statut','is_inter_centre','est_composition','heure_scan_professeur','heure_debut_pause','heure_fin_pause','durees_pauses_minutes','nb_presents_valide','cloture_validee_at','cloture_validee_par'];
    protected $casts = ['debut'=>'datetime','fin'=>'datetime','is_inter_centre'=>'boolean','est_composition'=>'boolean','heure_scan_professeur'=>'datetime','heure_debut_pause'=>'datetime','heure_fin_pause'=>'datetime','cloture_validee_at'=>'datetime'];
    ...
    /**
     * Calcule la durée réelle de présence du professeur (Scan -> Maintenant/Clôture)
     */
    public function calculerDureeEffective(): int
    {
        if (!$this->heure_scan_professeur) return 0;
        if ($this->cloture_validee_at) {
            $fin = $this->cloture_validee_at;
