...
<head>
    ...
    <style>
        ...
        .sidebar{width:240px;height:100vh;position:fixed;top:0;left:0;background:var(--fonce);color:var(--beige);display:flex;flex-direction:column;z-index:100;overflow:hidden;}
        .sidebar-logo{padding:20px 16px 12px;border-bottom:1px solid rgba(255,255,255,.1);flex-shrink:0;}
        .badge-erp{font-size:10px;background:var(--marron);padding:2px 8px;border-radius:20px;font-family:monospace;color:#fff;}
        .sidebar-logo h6{font-size:14px;margin:6px 0 2px;color:var(--beige);}
        ...
        .sidebar nav a{display:flex;align-items:center;gap:10px;padding:10px 16px;color:rgba(255,243,221,.8);text-decoration:none;font-size:13px;transition:all .2s;}
        ...
        .sidebar-section{padding:8px 16px 4px;font-size:10px;text-transform:uppercase;letter-spacing:.08em;opacity:.4;font-weight:600;margin-top:4px;}
        .centre-link{display:flex;align-items:center;gap:8px;padding:7px 16px 7px 28px;color:rgba(255,243,221,.65);text-decoration:none;font-size:12px;transition:all .2s;border-left:2px solid transparent;}
        ...
        .sidebar-footer{flex-shrink:0;padding:16px;border-top:1px solid rgba(255,255,255,.1);}
        .main-content{margin-left:240px;}
        .top-bar{background:#fff;border-bottom:1px solid rgba(0,0,0,.08);padding:16px 28px;display:flex;align-items:center;justify-content:space-between;}
        .top-bar h1{font-size:20px;font-weight:700;color:var(--fonce);margin:0;}
        .content-area{padding:24px 28px;}
        .stat-card{background:#fff;border-radius:16px;border:1px solid rgba(0,0,0,.06);padding:20px;display:flex;align-items:center;gap:16px;}
        .stat-icon{font-size:28px;}
        .stat-value{font-size:26px;font-weight:700;color:var(--fonce);}
        .stat-label{font-size:12px;color:var(--marron);}
        .table-gasa thead th{background:var(--fonce);color:var(--beige);font-size:13px;}
...
<div class="sidebar">
    ...
    <nav>
        ...
        <a href="{{ route('dashboard.centre', $c->id) }}"
           class="centre-link {{ request()->route('centreId') == $c->id ? 'active' : '' }}">
        ...
        <a href="{{ route('dashboard.centre',$cid) }}" class="{{ request()->routeIs('dashboard.centre')?'active':'' }}">
            <i class="bi bi-house"></i> Tableau de Bord
        </a>
        <a href="{{ route('etudiants.index',$cid) }}" class="{{ request()->routeIs('etudiants.*')?'active':'' }}">
            <i class="bi bi-mortarboard"></i> Étudiants
        </a>
        <a href="{{ route('professeurs.index',$cid) }}" class="{{ request()->routeIs('professeurs.*')?'active':'' }}">
            <i class="bi bi-person-badge"></i> Professeurs
        </a>
        <a href="{{ route('seances.index',$cid) }}" class="{{ request()->routeIs('seances.*')?'active':'' }}">
            <i class="bi bi-calendar3"></i> Planning
        </a>
        <a href="{{ route('scan.index',$cid) }}" class="{{ request()->routeIs('scan.*')?'active':'' }}">
            <i class="bi bi-qr-code-scan"></i> Scan Accès
        </a>
        <a href="{{ route('matieres.index',$cid) }}" class="{{ request()->routeIs('matieres.*')?'active':'' }}">
        ...
        <a href="#"><i class="bi bi-exclamation-triangle"></i> Interpellations
            <span class="ms-auto badge rounded-pill" style="font-size:9px;background:rgba(255,255,255,.2);">bientôt</span>
        </a>
        @endif
    </nav>
    ...
    <div class="sidebar-footer">
        ...
        <div style="font-size:11px;opacity:.6;margin-bottom:8px;">{{ auth()->user()->email }}</div>
